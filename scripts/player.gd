	...
	if model:
		...
		for child in model.find_children("*", "BoneAttachment3D", true, false):
			...
			for wpn in child.get_children():
				if wpn.name.begins_with("Weapon") and not "Sprite" in wpn.name:
					var cat := wpn.name.trim_prefix("Weapon")
					_weapon_nodes[cat] = wpn
					wpn.visible = false
		var shield_node := model.find_child("ShieldIcon", true, false)
		if shield_node is Sprite3D:
			_shield_sprite = shield_node
			_shield_sprite.texture = null
			_shield_sprite.visible = false
		var ws_node := model.find_child("WeaponSpriteGraphic", true, false)
		if ws_node is Sprite3D:
			_weapon_sprite = ws_node
...
func _setup_inventory() -> void:
	...
	# Starter armor/helm/shield for equip testing
	for sid in ["armor_cape", "headgear_cap", "shield_woodenbuckler"]:
		var si := _item_db.get(sid) as Item
		if si:
			inventory.add_item(si.duplicate())
...
func _apply_equip_stats() -> void:
	...
	# Effects loop: accumulate bonuses from all equipped items
	var bonus: Dictionary = {}
	var ls_total: float = 0.0
	var ms_total: float = 0.0
	var ele: Dictionary = {}
	var all_slots := [
		Inventory.EquipSlot.RIGHT_HAND, Inventory.EquipSlot.LEFT_HAND,
		Inventory.EquipSlot.HEAD, Inventory.EquipSlot.TORSO,
		Inventory.EquipSlot.RING_1, Inventory.EquipSlot.RING_2,
		Inventory.EquipSlot.AMULET,
	]
	for slot in all_slots:
		var eq := inventory.get_equipped(slot)
		if not eq:
			continue
		for fx in eq.effects:
			var t: String = fx.get("type", "")
			var v: float = fx.get("val", 0.0)
			match t:
				"hp_regen":
					bonus["hp_regen"] = bonus.get("hp_regen", 0.0) + v
				"mana_regen":
					bonus["mana_regen"] = bonus.get("mana_regen", 0.0) + v
				"max_hp":
					bonus["max_hp"] = bonus.get("max_hp", 0.0) + v
				"max_mana":
					bonus["max_mana"] = bonus.get("max_mana", 0.0) + v
				"movement_speed":
					bonus["movement_speed"] = bonus.get("movement_speed", 0.0) + v
				"critical_chance":
					bonus["critical_chance"] = bonus.get("critical_chance", 0.0) + v
				"spell_damage":
					bonus["spell_damage"] = bonus.get("spell_damage", 0.0) + v
				"stat_strength":
					bonus["stat_strength"] = bonus.get("stat_strength", 0) + int(v)
				"stat_agility":
					bonus["stat_agility"] = bonus.get("stat_agility", 0) + int(v)
				"stat_intelligence":
					bonus["stat_intelligence"] = bonus.get("stat_intelligence", 0) + int(v)
				"stat_vitality":
					bonus["stat_vitality"] = bonus.get("stat_vitality", 0) + int(v)
				"life_steal":
					ls_total += v
				"mana_steal":
					ms_total += v
				"fire_damage", "cold_damage", "lightning_damage", "poison_damage":
					ele[t] = ele.get(t, 0.0) + v
	stats.set_equip_bonus(bonus)
	_life_steal = ls_total
	_mana_steal = ms_total
	_bonus_elemental = ele
...
func _update_weapon_visibility() -> void:
	...
	# RIGHT_HAND weapon sprite (reemplaza 3D model para no-bow)
	if _weapon_sprite:
		if weapon and weapon.category != "Bow":
			_weapon_sprite.texture = weapon.get_texture()
			match weapon.category:
				"Sword":  _weapon_sprite.position.y = 0.30
				"Axe":    _weapon_sprite.position.y = 0.22
				"Mace":   _weapon_sprite.position.y = 0.20
				"Dagger": _weapon_sprite.position.y = 0.12
				_:        _weapon_sprite.position.y = 0.20
			_weapon_sprite.visible = true
			if _weapon_nodes.has(weapon.category):
				_weapon_nodes[weapon.category].visible = false
		else:
			_weapon_sprite.visible = false

	# LEFT_HAND shield
	if _shield_sprite:
		var shield := inventory.get_equipped(Inventory.EquipSlot.LEFT_HAND)
		if shield:
			_shield_sprite.texture = shield.get_texture()
...
func is_hovering_item() -> bool:
	return _hovered_item != null
...
func _process(delta: float) -> void:
	...
	if Input.is_key_pressed(KEY_F9):
		...
		_f9_held = false
	if not _is_stats_open() and not _is_inventory_open() and not _attacking and not Input.is_key_pressed(KEY_SHIFT):
	...
	# Mouse hover pickup for world items (Shift-gated)
	var can_pickup := Input.is_key_pressed(KEY_SHIFT) or _is_inventory_open()
	if not _is_stats_open():
		...
		if cam:
			...
			if can_pickup:
				...
				# Mouse item pickup: fresh click + no inventory drag
				var inv_scr := get_node_or_null("../InventoryScreen")
				if not inv_scr:
					inv_scr = get_node("/root/Main/InventoryScreen")
				var dragging: bool = inv_scr and inv_scr.visible and inv_scr.has_method("is_dragging") and inv_scr.is_dragging()
				var just_dropped: bool = inv_scr and inv_scr.has_method("is_just_dropped") and inv_scr.is_just_dropped()
				if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) and not _mouse_left_down and _hovered_item and not dragging and not just_dropped:
					var inv: Inventory = get_inventory()
					if _hovered_item.has_method("pickup_by_mouse"):
						_hovered_item.call("pickup_by_mouse", inv)
			else:
				if _hovered_item:
					if _hovered_item.has_method("set_hovered"):
						_hovered_item.call("set_hovered", false)
					_hovered_item = null

			# Mouse state tracking (runs regardless of can_pickup)
			if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
				_mouse_up_frames = 0
				_mouse_left_down = true
			else:
				_mouse_up_frames += 1
				if _mouse_up_frames > 1:
					_mouse_left_down = false

			# Cursor: hand when hovering world item, reset only when inventory closed
			if _hovered_item:
...
func _build_tracer_modulated() -> void:
	...
	# ═══════════ CONFIG ═══════════
	# ——— Tamaño ———
	var half_width: float = (weapon.trail_width * 0.5) if weapon else 0.275
	var trail_len:   int  = weapon.trail_max_samples if weapon else 5

	# ——— Color ———
	var color: Color = weapon.trail_color if weapon else Color(1.0, 1.0, 1.0)
	if weapon and weapon.color_modulate != Color.WHITE:
		color = weapon.color_modulate

	# ——— Opacidad ———
	var max_alpha: float = 0.5
	var fade_power: float = 2.0

	# ——— Debug ———
	#print("trail: color=", color, " len=", trail_len, " alpha_max=", max_alpha, " fade=", fade_power)

	# ——— Posición ———
	var offset: Vector3 = weapon.trail_offset if weapon else Vector3(0, 0.279, 0)
	var min_dist: float = 0.03
	# ════════════ END ════════════

	# Ring buffer
	var max_keep: int = trail_len
	while _trail_bases.size() > max_keep:
		_trail_positions.pop_front()
		_trail_bases.pop_front()
	...
	for i in n:
		var p: Vector3 = _trail_positions[i] + _trail_bases[i] * offset
		if kept > 0 and p.distance_to(last_p) < min_dist:
			continue
		var t: float = float(i) / max(n - 1, 1)
		var alpha: float = pow(t, fade_power) * max_alpha
		var c: Color = color
		c.a = alpha
		st.set_color(c)
		var blade: Vector3 = _trail_bases[i].y.normalized() * half_width
...
func _apply_sword_window() -> void:
	...
	for element in _bonus_elemental:
		dmg += _bonus_elemental[element]
	...
	for body in bodies:
		if body != self and not _sword_hit_enemies.has(body) and body.has_method("take_damage"):
			...
			if _mana_steal > 0.0:
				var ms := dmg * _mana_steal
				stats.mana = min(stats.mana + ms, stats.get_max_mana())
			if inventory and inventory.get_equipped(Inventory.EquipSlot.RIGHT_HAND):
				AudioManager.play_sfx("sword_impact")
			else:
...
func take_damage(amount: float) -> void:
	if stats:
		...
		_hp_regen_delay = 5.0
		AudioManager.play_sfx_channel3("player_hit")
